...
```bash
bash image/build.sh                       # builds sandboxd-base:0.4.0-nordineonline.1 (the default tag)
...
plane pins an exact version (`SANDBOXD_IMAGE`, default `sandboxd-base:0.4.0-nordineonline.1`). The
...
```bash
docker run --rm sandboxd-base:0.4.0-nordineonline.1 bash /opt/verify-base.sh
```

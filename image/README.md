...
```bash
bash image/build.sh                       # builds sandboxd-base:0.4.1-nordineonline.10 (the default tag)
...
plane pins an exact version (`SANDBOXD_IMAGE`, default `sandboxd-base:0.4.1-nordineonline.10`). The
...
```bash
docker run --rm sandboxd-base:0.4.1-nordineonline.10 bash /opt/verify-base.sh
```

...
```bash
bash image/build.sh                       # builds sandboxd-base:0.4.1-nordineonline.7 (the default tag)
...
plane pins an exact version (`SANDBOXD_IMAGE`, default `sandboxd-base:0.4.1-nordineonline.7`). The
...
```bash
docker run --rm sandboxd-base:0.4.1-nordineonline.7 bash /opt/verify-base.sh
```

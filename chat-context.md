...
| `0.4.1-nordineonline.9` | **Refonte sidebar** : suppression des entrées Brain / App Store / Feedback ; lien GitHub → fork ; liste des apps avec état (running en premier) + sous-menu par app (Start/Stop réels, Open, tous les onglets) ; suppression de la recherche/⌘K, de la barre d'onglets in-app et du bouton Deploy ; popup **QRCode URL** (lib `qrcode`). |
| `0.4.1-nordineonline.10` | **Ollama Cloud** câblé de bout en bout (Settings → Agents + listbox chat ; catalogue dynamique `GET /v1/agents/ollama/models` → 19 modèles sur `https://ollama.com/v1` ; exécution via clé). **GitHub Copilot** ajouté à la listbox providers du chat (visible, mais auth OAuth-device non proxyable → à utiliser dans l'onglet OpenCode natif). |
